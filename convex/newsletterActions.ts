...
export const sendWelcomeEmail = internalAction({
  ...
  handler: async (_ctx, args) => {
    ...
    const siteName = args.siteName || "Skill Stack";
    const siteUrl = args.siteUrl || "https://skillstack.md";
    const unsubscribeUrl = `${siteUrl}/unsubscribe?email=${encodeURIComponent(args.email)}&token=${args.unsubscribeToken}`;
    ...
    try {
      await client.inboxes.messages.send(inbox, {
        ...
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
            <h1 style="font-size: 24px; color: #1a1a1a; margin-bottom: 16px;">Welcome to ${escapeHtml(siteName)}</h1>
            <p style="font-size: 16px; color: #444; line-height: 1.6; margin-bottom: 16px;">
              You're in. Every week you'll get one AI skill you can copy, paste, and use immediately.
            </p>
            <p style="font-size: 16px; color: #444; line-height: 1.6; margin-bottom: 24px;">
              No fluff, no hype—just markdown files that make your writing faster and better.
            </p>
            <p style="margin-bottom: 32px;">
              <a href="${siteUrl}/skills" style="display: inline-block; padding: 12px 24px; background: #1a1a1a; color: #fff; text-decoration: none; border-radius: 6px; font-weight: 500;">Browse Skills</a>
            </p>
            ...
            <hr style="border: none; border-top: 1px solid #eee; margin: 32px 0;" />
            <p style="font-size: 12px; color: #888;">
              You received this because you subscribed to ${escapeHtml(siteName)}.<br />
              <a href="${unsubscribeUrl}" style="color: #888;">Unsubscribe</a>
            </p>
          </div>
        `,
        text: `Welcome to ${siteName}\n\nYou're in. Every week you'll get one AI skill you can copy, paste, and use immediately.\n\nNo fluff, no hype—just markdown files that make your writing faster and better.\n\nBrowse skills: ${siteUrl}/skills\n\n– Charlie\n\n---\nUnsubscribe: ${unsubscribeUrl}`,
      });
